...
Select7.add = (elem, e) => {
    ...
    $('#selectedTopics').val($('#selectedTopics').val() + option_text + ';'); // anas

    if (option_value === "filler" && option_text === "")    return;
...
Select7.remove = (elem, e) => {
    ...
    // by anas
    var Current_Selected_topics = $('#selectedTopics').val();
    //window.alert(now);
    var topics_arr = Current_Selected_topics.split(";");

    $('#selectedTopics').val('');
    var isfirst = 1;
    for(var i=0; i<topics_arr.length ; i++){
        if(topics_arr[i] == option_text){
          //dont take
        }
        else{
          if(isfirst == 0) topics_arr[i] =';' + topics_arr[i];
            $('#selectedTopics').val($('#selectedTopics').val() + topics_arr[i]);
            isfirst = 0;
        }
    }
